...
# exp dir name
spec_lbl=""
if [ "$apply_specaug" == "true" ]; then
    spec_lbl=sa
fi
if [ "$apply_tmask" == "true" ]; then
    spec_lbl=${spec_lbl}_${apply_tmask}
fi
...
exp_name=exp_${expid}_model_train_${trainer_ver}_${conv_label}_data_name_${data_name}_optim_${optim_type}_lr_${lr}_reg_${reg_label}_df_${data_format}_bs_${batch_size}_seq_len_${seq_len}_${loss_label}_${spec_lbl}
...
train_args="--setup-logger $setup_logger
        ...
        --save-freq $save_freq
        --specaug.apply $apply_specaug
        --specaug.time-mask $apply_tmask
